...
真的熱感印表機印字頭固定，紙（連同上面已經印好的整卷）一起往下走。這座鐘是倒掛的熱感頭：
新張跟著整捲紙帶一起下移，**尾巴（撕線那端）先探出出紙口，大字要到送紙尾聲才壓軸露臉**。
所以不做兩套動畫，新張也不是原地浮出來的特例：
...
大字落在收據上緣往下約 22%–42% 高度處（`L_BIG`..`REVEAL`）。尾巴先出的送紙模型下，
新張要等到快送完才會露出這一段，門檻是舊式子的鏡射：`fed >= RH − REVEAL*LH`（約 58% RH）。
2600 ms 的 ease-out 動畫裡約 650 ms 到位，仍遠低於 `simulate.js` 的 `SETTLE_MAX = 6000`。
不需要動 `simulate.js`。
...
紙帶末端不准留水平硬邊（切過兩次、退過兩次）——當年的解法是往背景色線性淡入；
改成整卷可見後，末端要嘛是末張自己的撕邊、要嘛垂出畫面外，硬邊問題連同淡出一起消失（見七）。
...
舊版在撕線前 45px 淡出到背景色收尾；橫式收據高 511px、撕線在 562px，末端只有 16px
落在淡出區，`THANK YOU` 那行會被淡掉，且要救得動 `HEAD` 或縮淡出帶都會把撕線推出畫面外。
後來拿掉淡出、`tape()` 的 clip 界改到畫面底，紙帶本來就該垂過撕線到畫面外——末端維持
乾淨撕邊即可，不用再靠淡出遮醜，這條瑕疵直接不存在了。
